...
def normalise(audio):
    ...
    elif audio.sample_width == 4 or audio.sample_width == 3:
        samples = samples.astype(np.float32)
...
def denormalise(float_samples, sample_width=2):
    ...
    elif sample_width == 4 or sample_width == 3:
...
def process_audio():
    ...
    if file_ext not in ALLOWED_EXTENSIONS:
        return "Invalid file type. Only WAV, MP3, and M4A files are supported.", 400
